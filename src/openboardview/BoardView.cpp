...
void BoardView::ThemeSetStyle(const char *name) {
	...
	if (strcmp(name, "dark") == 0) {
		...
		style.Colors[ImGuiCol_ModalWindowDimBg] =	ImVec4(1,1,1,0);
	...
	if (!filename.empty()) {
		...
		if (!buffer.empty()) {
			...
			if (file && file->valid) {
				 ...
				 *
				 */
				m_boarddb.NVGetStr("obdataBoardFile", &obdata_boardfile);
				if (obdata_boardfile.size() > 1) {
					obdata.load(obdata_boardfile);
...
void BoardView::ColorPreferencesItem(
	...
	cc = ImGui::ColorConvertU32ToFloat4(*c);
	c4[0] = cc.x;
	c4[1] = cc.y;
	c4[2] = cc.z;
	c4[3] = cc.w;
	...
	ImGui::SameLine();
	if (ImGui::ColorEdit4(butlabel, (float*)c4, (ImGuiColorEditFlags_HEX|ImGuiColorEditFlags_PickerHueBar|ImGuiColorEditFlags_NoSidePreview|ImGuiColorEditFlags_NoTooltip))) {
		cc.x = c4[0];
		cc.y = c4[1];
		cc.z = c4[2];
		cc.w = c4[3];
		*c = ImGui::ColorConvertFloat4ToU32(cc);
...
void BoardView::BoardPreferences(void) {
	...
	if (ImGui::BeginPopupModal("Board Preferences", &dummy, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		RA("OBData Board File", DPI(250)); ImGui::SameLine();
		ImGui::Text("%s", obdata_boardfile.c_str());
		ImGui::SameLine();
		if (ImGui::Button("Select")) {
			auto filename = show_file_picker();
			obdata_boardfile = filename;
			m_boarddb.NVSet("obdataBoardFile", obdata_boardfile);
			m_boarddb.NVGetStr("obdataBoardFile", &obdata_boardfile);
			obdata.load(obdata_boardfile);
			for ( auto &n: m_board->Nets() ) {
				for (auto m: obdata.data) {
...
#define KM(x) (((x)&0xFF) | 0x100)
	...
	void BoardView::DrawSelectedWeb(ImDrawList *draw) {
		...
		for (auto &p : m_pinHighlighted) {
			...
			draw->AddLine(CoordToScreen(fp.x, fp.y), CoordToScreen(p->position.x, p->position.y), ImU32(col), 1);
		}
